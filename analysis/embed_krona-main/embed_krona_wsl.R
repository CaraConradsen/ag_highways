...
to_wsl_path <- function(win_path) {
  p <- normalizePath(win_path, winslash = "/", mustWork = FALSE)
  # Extract drive letter
  drive <- tolower(substr(p, 1, 1))
  # Remove "C:" part
  path_no_drive <- substring(p, 3)
  # Build WSL path
  paste0("/mnt/", drive, path_no_drive)
}
                             ...
                             debug = F)
{
  ...
  if (is.null(kronatools_dir)) {
    r = suppressWarnings(try(system2('wsl',
      c('ktImportXML'),
...
args = c(
  '-o', to_wsl_path(outfile),
  if (!is.null(resources_url)) c('-u', resources_url) else NULL,
  to_wsl_path(xml_file)
)


# run inside WSL
ret = system2("wsl", c("ktImportXML", args), stdout = TRUE, stderr = TRUE)
...
# return the output (Windows path, so you can open in R/browser)
outfile
